...
from src.core.repositories.parameter_repository import ParameterRepository
...
class FilterParametersUseCase:
    ...
    def execute(self, request: FilterParametersRequest) -> FilterParametersResponse:
        ...
        try:
            ...
            all_parameters = self.parameter_repository.get_by_session(
                request.session_id)
            ...
            if not request.filter_criteria or self._is_empty_criteria(request.filter_criteria):
                self.logger.info(
                    "Пустые критерии фильтрации, возвращаем все параметры")
                parameter_dtos = [ParameterDTO.from_entity(
            ...
            filtered_parameters = self.filtering_service.filter_parameters(
                all_parameters, criteria)
            ...
            self.logger.info(
                f"Фильтрация завершена: {len(all_parameters)} -> {len(filtered_parameters)} параметров")
...
class FindChangedParametersUseCase:
    ...
    def execute(self, request: FindChangedParametersRequest) -> FindChangedParametersResponse:
        ...
        try:
            self.logger.info(
                f"Начат поиск изменяемых параметров: {request.from_time} - {request.to_time}")
            ...
            changed_params = self.data_model.find_changed_parameters_in_range(
                request.threshold)
            ...
            # Преобразуем в DTO
            changed_dtos = [ParameterDTO.from_entity(
                param) for param in filtered_changed_params]

            # Получаем статистику
            statistics = self.data_model.get_time_range_statistics()

            # Добавляем детальную статистику анализа
            detailed_analysis = self.data_model.analyze_parameter_changes_detailed(
                request.threshold)

            execution_time = (time.time() - start_time) * 1000

            self.logger.info(
                f"Поиск завершен: найдено {len(filtered_changed_params)} изменяемых параметров из {len(filtered_all_params)} ({execution_time:.1f}ms)")
...
class TimeRangeInitUseCase:
    ...
    def execute(self, request: TimeRangeInitRequest) -> TimeRangeInitResponse:
        ...
        try:
            ...
            self.logger.info(
                f"Временной диапазон инициализирован: {time_fields['from_time']} - {time_fields['to_time']}")
...
class ValidateTimeRangeUseCase:
    ...
    def execute(self, from_time: str, to_time: str) -> dict:
        ...
        try:
            ...
            temp_success = self.data_model.time_range_service.set_user_time_range(
                from_time, to_time)

            if not temp_success:
                validation_result['is_valid'] = False
                validation_result['errors'].append(
                    'Неверный формат времени или диапазон')
            ...
            if stats.get('coverage_percent', 0) < 1:
                validation_result['warnings'].append(
                    'Выбранный диапазон покрывает менее 1% данных')
            ...
            self.logger.debug(
                f"Валидация диапазона {from_time} - {to_time}: {'успешна' if validation_result['is_valid'] else 'неуспешна'}")
    ...
    def execute(self, parameter_type: str = 'all') -> dict:
        ...
        try:
            ...
            if parameter_type == 'normal':
                filtered_params = [
                    p for p in all_params if not p.is_problematic]
            ...
            # Преобразуем в DTO
            parameter_dtos = [ParameterDTO.from_entity(
                param) for param in filtered_params]
            ...
            self.logger.info(
                f"Получены параметры типа '{parameter_type}': {len(filtered_params)} из {len(all_params)}")
