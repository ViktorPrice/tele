...
class TimestampParameterService:
    ...
    def extract_timestamp_parameters(self, parameters: List[Parameter]) -> Dict[str, List[Parameter]]:
        ...
        for wagon in timestamp_params:
            timestamp_params[wagon] = self._sort_timestamp_components(timestamp_params[wagon])
...
class DataModel:
    ...
    def __init__(self):
        ...
        self.logger.info("DataModel (новая архитектура с timestamp и временными диапазонами) инициализирована")
    ...
    def _process_telemetry_data(self, telemetry_data: TelemetryData):
        ...
        try:
            ...
            if timestamp_integrated:
                self.logger.info("Timestamp функциональность успешно интегрирована")
            ...
            self._time_range_fields = self.time_range_service.initialize_from_telemetry_data(telemetry_data)
            ...
            if self._time_range_fields:
                ...
                self.logger.warning("Не удалось инициализировать поля времени")
    ...
    def get_time_range_fields(self) -> Dict[str, str]:
        """Получение значений полей 'от' и 'до' для UI"""
        return self._time_range_fields or {
            'from_time': '',
            'to_time': '',
            'duration': '',
            'total_records': 0
    ...
    def find_changed_parameters_in_range(self, threshold: float = 0.1) -> List[Parameter]:
        ...
        try:
            ...
            if cache_key in self._analysis_cache:
                self.logger.debug("Использование кэшированных результатов анализа")
    ...
    def reset_time_range_to_full(self):
        ...
        try:
            ...
            if self._telemetry_data:
                self._time_range_fields = self.time_range_service.initialize_from_telemetry_data(self._telemetry_data)
        ...
        try:
            ...
            if self._telemetry_data:
                ...
                if current_range:
                    ...
                    stats['records_ratio'] = (filtered_data.records_count / self._telemetry_data.records_count * 100) if self._telemetry_data.records_count > 0 else 0
        ...
        try:
            ...
            for param in self._cached_parameters:
                if param.full_column in filtered_data.data.columns:
                    ...
                    is_changed = self.time_range_service._is_parameter_changed(series, threshold)
