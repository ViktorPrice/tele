...
class ComponentFactory:
    ...
    def create_time_panel(self, parent, controller=None) -> 'TimePanel':
        """Создание РАБОЧЕЙ панели времени с поддержкой изменяемых параметров"""
        try:
            from ..components.time_panel import TimePanel

            time_panel = TimePanel(parent, controller)

            self.logger.info(
                "Создана РАБОЧАЯ панель времени с поддержкой изменяемых параметров")
            return time_panel

        except Exception as e:
            self.logger.error(f"Ошибка создания панели времени: {e}")
            # Возвращаем заглушку
            return self._create_placeholder(parent, "TimePanel")
    ...
    def create_filter_panel(self, parent, controller):
        ...
        try:
            ...
            panel = self._create_working_filter_panel(parent, controller)

            # Устанавливаем padding и размеры для приближения к старому виду
            panel.frame.configure(padding=(10, 5))
    ...
    def create_visualization_area(self, parent, controller):
        """Создание панели визуализации PlotVisualizationPanel"""
        try:
            from ..components.plot_visualization_panel import PlotVisualizationPanel
            panel = PlotVisualizationPanel(parent, controller)
            return panel
        except Exception as e:
            self.logger.error(f"Ошибка создания PlotVisualizationPanel: {e}")
            return None
    ...
    def _create_working_filter_panel(self, parent, controller):
        ...
        ttk.Button(btn_frame, text="Сбросить все",
                   command=toggle_all).grid(row=0, column=0, padx=5)
        ttk.Button(btn_frame, text="Инвертировать",
                   command=invert_all).grid(row=0, column=1, padx=5)
            ...
            def update_line_checkboxes(self, lines):
                ...
                for i, line in enumerate(lines):
                    ...
                    cb = ttk.Checkbutton(
                        ...
                        variable=var,
                        command=lambda: controller.apply_filters() if hasattr(
                            controller, 'apply_filters') else None
                    )
                    cb.grid(row=i//2, column=i % 2, sticky="w", padx=2, pady=1)

                self.logger.info(
                    f"Обновлены чекбоксы линий: {len(lines)} линий")
    ...
    def _create_working_parameter_panel(self, parent, controller):
        ...
        title_label = ttk.Label(
            main_frame, text="Панель параметров", font=('Arial', 12, 'bold'))
        ...
        all_tree = ttk.Treeview(all_frame, columns=(
            "signal", "desc", "line", "wagon"), show="headings")
        ...
        all_scroll = ttk.Scrollbar(
            all_frame, orient="vertical", command=all_tree.yview)
        ...
        ttk.Button(btn_frame, text="Добавить →", command=add_selected_params).grid(
            row=0, column=0, pady=5, sticky="ew")
        ttk.Button(btn_frame, text="← Удалить", command=remove_selected_params).grid(
            row=1, column=0, pady=5, sticky="ew")
        ttk.Button(btn_frame, text="Очистить", command=clear_selected_params).grid(
        ...
        selected_tree = ttk.Treeview(selected_frame, columns=(
            "signal", "desc", "line", "wagon"), show="headings")
        ...
        class ParameterPanelWrapper:
            def __init__(self, frame, all_tree, selected_tree):
                ...
                self.selected_params_tree = type(
                    'Tree', (), {'tree': selected_tree})()
            ...
            def update_tree_all_params(self, params):
                ...
                self.logger.info(
                    f"Обновлено дерево всех параметров: {len(params)} параметров")
            ...
            def update_tree_selected_params(self, params):
                ...
                for param in params:
                    if isinstance(param, dict):
                        ...
                        values = param
                    self.selected_params_tree.tree.insert(
                        '', 'end', values=values)
    ...
    def _safe_call(self, controller, method_name):
        ...
        try:
            if hasattr(controller, method_name):
                ...
                self.logger.warning(
                    f"Метод {method_name} не найден в контроллере")
        except Exception as e:
